...
class Mekanik extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model("masterdata/mekanik_model");
    ...
    {
            $data = $this->mekanik_model->getCabang($this->input->post('kode'));
            echo json_encode($data);
    }

    public function getforeman()
    {
            $data = $this->mekanik_model->getforeman($this->input->post('kode'));
            echo json_encode($data);
    }
    ...
    public function update()
    {
        ...
        $data = array(
            ...
            'kode_foreman' => $this->input->post('kodeforeman'),
            'aktif' => $this->input->post('aktif'),
            'tglsimpan' => date("Y-m-d H:i:s"),
            // 'pemakai' => $this->session->userdata('myusername')
            'pemakai' => $userlogin
        );
            $result = $this->mekanik_model->update($data,$this->input->post('kode'));
            if($result == true){
                $resultjson = array(
    ...
    {
            $data = $this->mekanik_model->get($this->input->post('kode'));
            echo json_encode($data);
    }
    public function save()
    {
        $userlogin = $this->session->userdata('myusername');
        $get["mekanik"] = $this->mekanik_model->get($this->input->post('kode')); 
		if (!$get["mekanik"]){
            $data = array(
                'kode' => $this->input->post('kode'),
                'nama' => $this->input->post('nama'),
            ...
            $result =  $this->mekanik_model->save($data);
            if($result == true){
                $resultjson = array(
                    'kode' => $this->input->post('kode'),
            ...
            foreach($this->input->post('field') as $key => $value){
                if ($i <= $count){
                    if ($i == 1){
                        $msearch = $row->$value;
                        // $sub_array[] = '<button class="btn btnt-info btn-xs searchcabang" data-id="'.$msearch.'"><i class="fas fa-plus"></i>&nbsp;<i class="fas fa-angle-double-right"></i></button> ';  
                        $sub_array[] = '<button class="btn btn-success searchforeman" data-id="'.$msearch.'"><i class="fa fa-hand-o-right"></i></button> ';  
                        $sub_array[] = $row->$value;
